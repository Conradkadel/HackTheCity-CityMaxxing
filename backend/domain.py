...
def response_payload(rows,version,areas,ops,start,end):
    observations=[dict(timestamp=int(r['created_at'].timestamp()*1000),receivedTimestamp=int(r['received_at'].timestamp()*1000),operatorId=r['agency_id'],vehicleId=r['vehicle_id'],tripId=r['trip_id'],stopId=r['stop_id'],latitude=r['latitude'],longitude=r['longitude'],geohash=r['geohash_5']) for r in rows]
